<?php
	...
	for($i=0; $i<1600; $i++)
	{
		...
		imageline($im,$rand1,$rand2,$rand1,
		$rand2, $siatka);
	}
	#losowanie pozycji znaków
	$x = rand(5, $szerokosc/(7/2));
	#dodawanie obramowania
	imagerectangle($im, 0, 0, $szerokosc-1,
	$wysokosc-1, $ramka);
	#umieszczanie liter na obrazku
	for($a=0; $a < $ilosc_znakow+1; $a++)
	{
		imagestring($im, $ilosc_znakow, $x, rand(4,
		$wysokosc/5),substr($string,$a,1), $czcionka);
